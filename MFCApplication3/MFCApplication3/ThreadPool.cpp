...
ThreadPool::Thread *ThreadPool::GetIdleThread()
{
	...
	{
		thread = new Thread(this);
		InterlockedIncrement((volatile LONG*)&maxNumOfThread);
	}
	return thread;
}
...
void ThreadPool::MoveBusyThreadToIdleList(Thread * busyThread)
{
	...
	for (list<ThreadPool::Thread*>::iterator it = busyThreadList.begin(); it != busyThreadList.end(); it++)
	{
		if (*it == busyThread)
		{
...
void ThreadPool::GetTaskExcute()
{
	...
	if (waitTask->bLong)
	{
		...
		{
			thread = new Thread(this);
			InterlockedIncrement((volatile LONG*)&numOfLongFun);
			InterlockedIncrement((volatile LONG*)&maxNumOfThread);
		}
	}
	else
